# layer2_validate.py
# Layer 2 §4 synthetic-data validation harness, Thread A — v0.4 stage.
# v0.4 carries the v0.1 calibration patch, the v0.2 V_prior sweep harness,
# and the v0.3 beta_x M-step. The single change relative to v0.3: the
# transition-matrix M-step in em_one is deleted, freezing log_A at the
# canonical generator values [[0.95, 0.05], [0.20, 0.80]] throughout EM.
# This is a COUNTERFACTUAL diagnostic, not a candidate production model —
# the production spec cannot freeze A at truth because truth is unknown.
# v0.4's purpose is to test whether the v0.3 Hamming(T) plateau is driven
# by the EM's inability to identify A on always-T data.
# Sweep configuration is unchanged from v0.3: default cell (beta_sell=0.3,
# a_chan=0.6), 5 V_prior values (0.01, 0.1, 1.0, 4.0, 64.0), 60 executives
# per cohort. See README.md for the v0.3 BETA_X_NULL hypothesis being tested.
# Single-command reproduction: python run.py --seed 20260426
    ...
    for V_prior in vprior_values:
        ...
        for cohort, regime in [('T','T'), ('C','C'), ('switch','switch')]:
            ...
            for i in range(60):
                ...
                h = min(np.mean(z_hat != d['z']), np.mean(z_hat != (1 - d['z'])))
                mh.append(h)
                mu_err.append(((fit['mu'][0] - 0.0)**2 + (fit['mu'][1] - 0.4)**2) / 2)
                if cohort == 'switch':
                    gamma_C = np.exp(fit['log_gamma'])[:, 1]
                    if np.mean(z_hat != (1 - d['z'])) < np.mean(z_hat != d['z']):
                        gamma_C = 1 - gamma_C
                    ...
                    for b in range(10):
                        m = idx == b
                        if m.sum() > 0:
                            ece += m.mean() * abs(gamma_C[m].mean() - (d['z'][m] == 1).mean())
                    me.append(ece)
                ms_w.append(diag['multistart_winners'] / 10.0)
                ents.append(np.mean(diag['entropies']))
            cohort_metrics[cohort] = dict(
                hamming=float(np.mean(mh)),
                mu_rmse=float(np.sqrt(np.mean(mu_err))),
                multistart=float(np.mean(ms_w)),
                entropy=float(np.mean(ents)),
                ece=float(np.mean(me)) if me else None,
            )
...
def main():
    ...
    # Markdown summary: rows = V_prior, columns = Hamming(switch), Hamming(T),
    # Hamming(C), ECE, mu-RMSE, multistart, entropy. mu-RMSE and multistart and
    # entropy are reported on the switching cohort (the default-cell focal
    # cohort); per-cohort numbers are in results.json.
    lines = ['# Layer 2 v0.2 — V_prior sweep results',
             f'seed={args.seed}, elapsed={elapsed:.1f}s',
             'Default cell: beta_sell=0.3, a_chan=0.6.',
             'mu-RMSE / multistart / entropy reported on the switching cohort; per-cohort numbers are in results.json.',
             '',
             '| V_prior | Hamming(switch) | Hamming(T) | Hamming(C) | ECE | mu-RMSE | multistart | entropy |',
             '|---|---|---|---|---|---|---|---|']
    for r in results:
        sw, tr, co = r['switch'], r['T'], r['C']
        lines.append(
            f"| {r['V_prior']} | {sw['hamming']:.3f} | {tr['hamming']:.3f} | {co['hamming']:.3f} | "
            f"{sw['ece']:.3f} | {sw['mu_rmse']:.3f} | {sw['multistart']:.3f} | {sw['entropy']:.3f} |"
        )
